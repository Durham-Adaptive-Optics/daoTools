...
class Main(QMainWindow, Ui_MainWindow):
    def __init__(self, shmimName, maskName):
        ...
        self.shmmask = daoShm.shm(maskName)
        ...
        self.mask = self.shmmask.get_data()
        self.mask[self.mask!=-1] = 1
        self.mask[self.mask==-1] = 0
        # Get first image
        slopes = self.shmim.get_data()[:,0]
        im=self.mask*0.0
        im[self.mask==1] = slopes
...
if __name__ == '__main__':
    ...
    maskName = '/tmp/pixidMap.im.shm'
    try:
        opts, args = getopt.getopt(sys.argv[1:],"hs:m:",["help", "shmimName=", "maskName="])
    except getopt.GetoptError:
      print('err, usage: daoSlopesDisp.py -s <shmimName>')
      sys.exit(2)
    print(opts)
    print(args)
    for opt, arg in opts:
        if opt == '-h':
            print('daoSlopesDisp.py -s <shmimName>')
            sys.exit()
        elif opt in ("-s", "--shm"):
            shmimName = str(arg)
        elif opt in ("-m", "--mask"):
